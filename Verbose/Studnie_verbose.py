...
class Path:
    ...
    def set_coordinates(self, i):
        i += 1
        start_x = int(input("Podaj współrzedną x początku {0}. punktu: ".format(i)))
        start_y = int(input("Podaj współrzedną y początku {0}. punktu: ".format(i)))
        end_x = int(input("Podaj współrzedną x końca {0}. punktu: ".format(i)))
        end_y = int(input("Podaj współrzedną y końca {0}. punktu: ".format(i)))
...
def collect_all_paths():
    L = int(input("Podaj liczbę ścieżek: "))
    paths = []
...
def studnie():
    ...
    print("\n")
    print("Aby ochronić wszystkie studnie, należy wystawić {0} posterunków.".format(counter))
